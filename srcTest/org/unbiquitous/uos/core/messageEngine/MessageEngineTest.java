...
public class MessageEngineTest {
	...
	// Negative cases
	@Test
	public void handleIncomingMessage_returnNullForNullMessage() throws Exception {
		assertNull(engine.handleIncomingMessage(null, mock(NetworkDevice.class)));
	}

	@Test
	public void handleIncomingMessage_returnNullForNullDevice() throws Exception {
		assertNull(engine.handleIncomingMessage("", null));
	}

	@Test
	public void handleIncomingMessage_returnErrorForMalformedJson() throws Exception {
		JsonNode response = mapper.readTree(engine.handleIncomingMessage("not a json", mock(NetworkDevice.class)));
		assertTrue(response.has("error"));
		assertFalse(response.get("error").asText().isEmpty());
	}

	@Test
	public void handleIncomingMessage_returnErrorFoMessageWithoutKnowType() throws Exception {
	...
	@Test
	public void handleIncomingMessage_delegateServiceCallToHandler() throws Exception {
		ObjectNode call = mapper.getNodeFactory().objectNode();
		...
		when(callHandler.handleServiceCall(any(Call.class), any(CallContext.class)))
				.thenReturn(new Response().addParameter("bla", "0"));

		JsonNode response = mapper.readTree(engine.handleIncomingMessage(call.toString(), caller));

		assertFalse(response.has("error"));
		assertEquals("SERVICE_CALL_RESPONSE", response.get("type").asText());
		assertEquals("0", response.get("responseData").get("bla").asText());

		ArgumentCaptor<Call> callCatcher = ArgumentCaptor.forClass(Call.class);
	...
	@Test
	public void handleIncomingMessage_returnErrorWhenServiceCallHandlerFails() throws Exception {
		ObjectNode call = mapper.getNodeFactory().objectNode();
		call.put("type", "SERVICE_CALL_REQUEST");
		call.put("driver", "my.driver");
		call.put("service", "my.service");
		NetworkDevice caller = mock(NetworkDevice.class);
		when(callHandler.handleServiceCall(any(Call.class), any(CallContext.class))).thenThrow(new RuntimeException());
	...
	// SERVICE NOTIFY
	@Test
	public void handleIncomingMessage_delegateNotifyToHandler() throws Exception {
		ObjectNode call = mapper.getNodeFactory().objectNode();
	...
	@Test
	public void handleIncomingMessage_ErrorsDuringEventsDontGenerateConsequences() throws Exception {
		ObjectNode call = mapper.getNodeFactory().objectNode();
		call.put("type", "NOTIFY");
		call.put("driver", "my.driver");
		call.put("eventKey", "my.event");
	...
	@Test
	public void handleIncomingMessage_AnEncapsulatedMessageMustBeDelegatedToTheAppropriateKindOfHanlderAfterTranslation_ServiceCall()
			throws Exception {
		ObjectNode call = mapper.getNodeFactory().objectNode();
		call.put("type", "ENCAPSULATED_MESSAGE");
		call.put("innerMessage", "my.msg");
		call.put("securityType", "abacate");
		ObjectNode innerCall = mapper.getNodeFactory().objectNode();
		...
		when(callHandler.handleServiceCall(any(Call.class), any(CallContext.class))).thenReturn(new Response());
		...
		assertEquals("ENCAPSULATED_MESSAGE", response.get("type").asText());
		assertEquals("abacate", response.get("securityType").asText());
		assertEquals("my.return", response.get("innerMessage").asText());
	}

	@Test
	public void handleIncomingMessage_AnEncapsulatedMessageMustBeDelegatedToTheAppropriateKindOfHanlderAfterTranslation_Notify()
			throws Exception {
		ObjectNode call = mapper.getNodeFactory().objectNode();
		call.put("type", "ENCAPSULATED_MESSAGE");
		call.put("innerMessage", "my.msg");
	...
	@Test
	public void handleIncomingMessage_AnEncapsulatedMessageNeverGenerateAnReturnInCaseOfError() throws Exception {
		ObjectNode call = mapper.getNodeFactory().objectNode();
		call.put("type", "ENCAPSULATED_MESSAGE");
		call.put("innerMessage", "my.msg");
		call.put("securityType", "abacate");
	...
	// @Test public void notifyEvent_delegatesToMessageHandles() throws
	// Exception{
	...
	// @Test public void callService_delegatesToMessageHandles() throws
	// Exception{
