 ...
 * devices and defining which type are they, then encapsulating it in the
 * appropriate format.
 * 
...
public class MessageEngine implements MessageListener, UOSComponent {
	...
	private String handleServiceCall(String message, CallContext messageContext) throws MessageEngineException {
		try {
			Call serviceCall = mapper.readValue(message, Call.class);
		...
		try {
			...
			notifyHandler.handleNofify(notify,
					deviceManager.retrieveDevice(
							connectionManagerControlCenter.getHost(clientDevice.getNetworkDeviceName()),
	...
	private String handleEncapsulatedMessage(String message, NetworkDevice clientDevice) throws MessageEngineException {
		try {
			Capsule encapsulatedMessage = mapper.readValue(message, Capsule.class);
			...
			// TODO: MessageEngine : This is a violation of responsibility among
			// the layers. Names should be already in the correct format
			UpDevice upClientDevice = deviceManager.retrieveDevice(
					connectionManagerControlCenter.getHost(clientDevice.getNetworkDeviceName()),
					clientDevice.getNetworkDeviceType());
			...
			if (innerResponse != null) {
				...
				return mapper.writeValueAsString(encapsulatedResponse);
			}
		} catch (Exception e) {
	...
	private Message.Type retrieveMessageType(ObjectNode json) {
		...
		if (type != null) {
			try {
				return Enum.valueOf(Message.Type.class, type.asText());
	 ...
	 * @param device
	 *            Device which is going to receive the notofy event
	...
	public void init(UOSComponentFactory factory) {
		this.serviceCallHandler = factory.get(AdaptabilityEngine.class);// FIXME:
																		// AdaptabilityEngine
																		// should
																		// register
		this.notifyHandler = factory.get(AdaptabilityEngine.class);// FIXME:
																	// AdaptabilityEngine
																	// should
																	// register
		// this.deviceManager = factory.get(DeviceManager.class);// FIXME:
		// DeviceManager should register
		...
		MessageHandler messageHandler = new MessageHandler(properties, connectionManagerControlCenter, securityManager,
