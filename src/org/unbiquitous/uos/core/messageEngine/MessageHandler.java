...
public class MessageHandler {
	...
	public MessageHandler(InitialProperties bundle, ConnectionManagerControlCenter connectionManagerControlCenter,
			SecurityManager securityManager, ConnectivityManager connectivityManager) {
	...
	public Response callService(UpDevice device, Call serviceCall) throws MessageEngineException {
		if (device == null || serviceCall == null || serviceCall.getDriver() == null
				|| serviceCall.getDriver().isEmpty() || serviceCall.getService() == null
				|| serviceCall.getService().isEmpty()) {
			throw new IllegalArgumentException("Either the Device or Service is invalid.");
		}

		try {
			JsonNode jsonCall = mapper.valueToTree(serviceCall);
			...
			if (returnedMessage != null)
				return mapper.readValue(returnedMessage, Response.class);
		} catch (Exception e) {
	...
	private String sendEncapsulated(String message, String securityType, UpDevice target) throws Exception {
		...
		message = mapper.writeValueAsString(new Capsule(securityType, message));
		message = send(message, target, true);

		Capsule e = mapper.readValue(message, Capsule.class);
	...
	public void notifyEvent(Notify notify, UpDevice device) throws MessageEngineException {
		if (device == null || notify == null || notify.getDriver() == null || notify.getDriver().isEmpty()
				|| notify.getEventKey() == null || notify.getEventKey().isEmpty()) {
			throw new IllegalArgumentException("Either the Device or Service is invalid.");
		}
		try {
			String message = mapper.writeValueAsString(notify);
	...
	private String send(String message, UpDevice target, boolean waitForResponse) throws Exception {
		...
		ClientConnection connection = connectionManagerControlCenter.openActiveConnection(netInt.getNetworkAddress(),
				netInt.getNetType());
